...
var transNonterm = make([]int, TEMPSIZE) // states generated by the transition non-term symbols in closure of an existing state
...
func stategen() {
	nstate = 0
	tstates = make([]int, termN+1)
	ntstates = make([]int, nontermN)
	clkset = newLkset()
	addKernItem(item{0, allPrds[0], 0, newLkset()})
	nstate++
	for i := 0; i < nstate; i++ {
		fill(transNonterm, nontermN, 0)
		closure(i)
		for j := 0; j < cwp; j++ {
			w := wSet[j]
			if w.done {
